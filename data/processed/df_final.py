...
## Determina se é final de semana ou dia de semana
...
df_final_cross['dist'] = df_final_cross.apply(lambda row: calcula_distancia(row['latitude_x'], row['latitude'], row['longitude_x'], row['longitude']), axis=1, result_type='expand')
...
df_final['dia_da_semana'] = df_final_cross.apply(lambda row: feriado_fds(row['data']), axis=1, result_type='expand')
...
df_final_dummies.to_csv('df_final_dummies.csv')
