...
for cidade in [c for c in parametros.cidades if c != 'santo antonio do amparo']:
    data_path1 = current_dir / '..' / 'raw' / f'dados_{cidade}'
    if cidade == 'betim':
        for i in range(12):
            if i == 0:
                raw_path = data_path1 / 'data.csv'
                estacoes_path = data_path1 / 'estacoes.txt'
...
data_path2 = str(current_dir / '..' / 'raw' / 'banco_santoantonio.mdb')
...
df_santoantonio['Data_modificada'] = df_santoantonio.apply(lambda row: alterar_ou_remover(row), axis=1)
